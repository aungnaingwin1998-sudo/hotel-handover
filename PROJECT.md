...
Create app/routers/auth.py (draft content already exists — see chat history or ask to regenerate it)
Add auth.router to main.py:
python
   from .routers import users, auth
   ...
   app.include_router(auth.router)
Test in /docs: log in with the test user from piece #1, confirm a JWT token comes back (access_token, token_type: "bearer")
Commit: "Piece 2: login endpoint (JWT), tested via /docs"
Once confirmed working → move to Piece #3 (Start shift)
...
3	Start shift	FR6–FR8, NFR5	45–60 min	⬜ Not started
