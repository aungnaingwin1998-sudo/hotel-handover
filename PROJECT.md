...
- Automatic shift open/close based on system time (cron/scheduler)
- Frontend UI (this phase is backend/API only)
- **Admin/user management (deletion or deactivation of accounts):** surfaced during a data-integrity discussion (fixing `ondelete` behavior on foreign keys). Not designed in detail — deferred to v2, would need its own session covering roles, permissions, and what "admin" actually means for this app.
- **Shift scheduling/assignment (considered and deliberately deferred):** discussed whether the system should know who's *scheduled* to work a given shift in advance, vs. just whoever logs in and clicks "Start Shift." Since Aung's actual hotel schedule changes based on operational need (covering for sick coworkers, swaps, etc.), a rigid "only the scheduled person can act" system would work against real flexibility. Decision: v1 stays with "whoever's logged in can start/close/add notes to any shift" — no `role` column, no assignment logic. A purely informational (non-enforcing) roster/calendar could be a v2 idea if ever needed.
...
│       ├── users.py                 ✅ done (register)
│       ├── shifts.py                  ✅ done (start shift, get current shift, close shift)
│       └── notes.py                    ✅ done (create note, acknowledge note)
```
...
| 6 | Close shift (`POST /shifts/close`) | ✅ **Done, tested, committed** |
| 7 | Shift history (`GET /shifts/history`) | ✅ **Done, tested, committed** — includes embedded notes |
| 8 | Acknowledge note (`PATCH /notes/{note_id}/acknowledge`) | ✅ **Done, tested, committed** |
...
| 8 | Acknowledge note | FR16–FR17 | 20–30 min | ✅ Done & committed |
...
**Where we stopped (pick up here next time):** 🎉 **All 8 pieces are done, tested, and committed — the v1 backend is complete.** Full lifecycle verified end-to-end: register → login → start shift → add notes → close shift (blocked without summary) → shift history (with embedded notes) → acknowledge notes (with safeguards against not-found, double-acknowledgment, and self-authorship).

**Next up, per the "After all 8 pieces" table below:**
1. Full flow test — one clean walkthrough of the entire journey in a single session, register through acknowledge, to catch anything that only shows up end-to-end
2. Polish — consider adding `GET /users/me`, review the NFR checklist (Section 11) against what's actually built
3. Decision point — frontend (Jinja2 templates, per earlier mockups) vs. API-only portfolio vs. deploy (Render/Railway) — decide once ready to move past pure backend work
| Date/step | What happened |
|---|---|
| Design | Discussed flow-first: shift lifecycle, note lifecycle, acknowledgment, history browsing |
| Decision | 12-hour fixed shifts (9–9), manual start/close (not auto-scheduled) |
